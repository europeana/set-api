...
//						httpStatus = HttpStatus.GONE;	
	...
	@RequestMapping(value = { "/set/filter/{identifier}", "/set/filter/{identifier}.jsonld" }, 
			method = {RequestMethod.GET},
			produces = {  HttpHeaders.CONTENT_TYPE_JSONLD_UTF8, HttpHeaders.CONTENT_TYPE_JSON_UTF8 }
			)
	@ApiOperation(notes = SwaggerConstants.SEARCH_HELP_NOTE, value = "Retrieve a user set for provided fields", 
				nickname = "retrieve filtered", response = java.lang.Void.class)
	public ResponseEntity<String> getUserSetFiltered(
			@RequestParam(value = WebUserSetFields.PARAM_WSKEY) String wskey,
			@PathVariable(value = WebUserSetFields.PATH_PARAM_SET_ID) String identifier,
			@RequestParam(value = WebUserSetFields.PARAM_QF, required = false) String[] filters,
			@RequestParam(value = WebUserSetFields.USER_TOKEN, required = false, defaultValue = WebUserSetFields.USER_ANONYMOUNS) String userToken,			
			@RequestParam(value = WebUserSetFields.PROFILE, required = false, defaultValue = WebUserSetFields.PROFILE_MINIMAL) String profile,			
			HttpServletRequest request) throws HttpException {
		...
		return getUserSetFiltered(wskey, ldProfile, identifier, filters, request, action);
	}
	 ...
	 * @param profile The profile definition
	 * @param identifier The identifier
	 * @param filters The response is limited by these fields
	 * @param request HTTP request
	 * @param action The action describing the request
	 * @return response entity that comprises response body, headers and status code
	 * @throws HttpException
	 */
	private ResponseEntity<String> getUserSetFiltered(String wsKey, LdProfiles profile, String identifier, 
			String[] filters, HttpServletRequest request, String action)
					throws HttpException {
		try {
			// check user credentials, if invalid respond with HTTP 401.
			// check client access (a valid "wskey" must be provided)
			...
			// if the Set doesn’t exist, respond with HTTP 404
			// if the Set is disabled respond with HTTP 410
			UserSet userSet = getUserSetService().getUserSetByIdAndFilters(identifier, filters);
			String userSetJsonLdStr = serializeUserSet(profile, userSet); 
