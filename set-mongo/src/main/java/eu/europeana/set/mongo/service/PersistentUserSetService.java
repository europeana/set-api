...
public interface PersistentUserSetService extends AbstractNoSqlService<PersistentUserSet, String> {
  ...
  /**
   * This method checks if a user set with provided type and creator already exists in database
   * 
   * @param creatorid
   * @return user set object
   */
  ...
  /**
   * verifies if the given user set is a duplicate of an existing set. Applicable only for
   * BestItemsUserSet
   * 
  ...
  /**
   * This method generates the id for the UserSet and stores it into the database 
   * @param userSet object to be stored
